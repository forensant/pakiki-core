// This is the official interactsh client, available at: "github.com/projectdiscovery/interactsh/pkg/server", with two changes:
//   1. The client object can be serialised and deserialised, for storage within the project.
//   2. If an error occurs, it is not fatal to the entire application.
...
func ClientFromJSON(json string) (*Client, error) {
	...
	parsedUrl, err := url.Parse(jsonObj.ServerURL)
	if err != nil {
		return nil, err
	}
	...
	return &Client{
		...
		correlationID:     jsonObj.CorrelationID,
		...
		for {
			...
			case <-ticker.C:
				...
				if err != nil && err.Error() == authError.Error() {
					fmt.Printf("Could not authenticate to the interactsh server\n")
					ticker.Stop()
					return
				}
			case <-c.quitChan:
